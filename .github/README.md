# ElroyMart API
This is the ElroyMart API that should be used to route requests for products, categories, and images, published on a seperate repository to not publish the whole source code. 

## Contributions
### If you do not know how to use GitHub or write products in JSON:
[Create a new issue](https://github.com/BuniMart/ElroyMart-API/issues/new/choose) and fill in the appropriate template.
### If you know what you're doing:
Fork this repository, add your changes and make a pull request.
#### Note for people who want to directly contribute:
Do not force push this repository unless you ***really*** know what you are doing, please use pull requests instead to resolve changes. 
